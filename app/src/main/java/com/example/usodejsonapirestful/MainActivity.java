...
public void Btingresar (View view){
    EditText Nombre = findViewById(R.id.email);
    ...
    public void processFinish(String result) throws JSONException {
        TextView resultado=findViewById(R.id.Respuesta);
        JSONObject jsonresultado=new JSONObject(result);
        if (jsonresultado.has("error"))
        {
            resultado.setText(jsonresultado.getString("error"));
        }
        else
        {

            Bundle b = new Bundle();
            b.putString("TOKEN", jsonresultado.getString("access_token"));
            Intent intent = new Intent(MainActivity.this,LstBancos.class);
            intent.putExtras(b);
